# Mayraju | Backend Engineer

I design and build backend services that are fast, reliable, and easy to scale.

---

## 👨‍💻 About Me

Backend developer with **3+ years of experience** improving API response times, optimizing background processing, and building scalable systems — mainly in **healthcare and fintech domains**.

I focus on writing clean backend code, designing reliable systems, and solving real production problems.

---

## 🔭 What I’m Working On

- Backend development for **healthcare platforms**  
  (secure REST APIs, authentication flows, webhook-based integrations)
- Performance optimization  
  (async processing, background jobs, API latency reduction)
- Preparing for **DSA and system design interviews**

---

## 🌱 Currently Improving

- Data Structures & Algorithms  
- Low-level Design (LLD) & High-level Design (HLD)  
- Distributed systems fundamentals  

---

## ✉️ Contact

- **LinkedIn:** https://www.linkedin.com/in/mayraju-p-09aa50109  
- **Email:** mayraju777@gmail.com  

---

## 💻 Coding Profiles

- **LeetCode:** https://leetcode.com/  
- **GeeksforGeeks:** https://auth.geeksforgeeks.org/  
- **GitHub:** https://github.com/Mayraju  

---

## 🛠️ Tech Stack

### Languages
[![Java](https://img.shields.io/badge/Java-white?style=flat&logo=java&logoColor=007396)](https://www.java.com/)
[![C](https://img.shields.io/badge/C-white?style=flat&logo=c&logoColor=A8B9CC)](https://en.wikipedia.org/wiki/C_(programming_language))
[![Python](https://img.shields.io/badge/Python-white?style=flat&logo=python&logoColor=3776AB)](https://www.python.org/)
[![Go](https://img.shields.io/badge/Go-white?style=flat&logo=go&logoColor=00ADD8)](https://golang.org/)

### Backend & Frameworks
- Spring Boot  
- Node.js (Express, REST APIs)  
[![Spring Boot](https://img.shields.io/badge/Spring_Boot-white?style=flat&logo=springboot&logoColor=6DB33F)](https://spring.io/projects/spring-boot)
[![Node.js](https://img.shields.io/badge/Node.js-white?style=flat&logo=node.js&logoColor=339933)](https://nodejs.org/)

### Databases
- MySQL  
- MongoDB  
- SQL  

[![MySQL](https://img.shields.io/badge/MySQL-white?style=flat&logo=mysql&logoColor=4479A1)](https://www.mysql.com/)
[![MongoDB](https://img.shields.io/badge/MongoDB-white?style=flat&logo=mongodb&logoColor=47A248)](https://www.mongodb.com/)
[![SQL](https://img.shields.io/badge/SQL-white?style=flat&logo=postgresql&logoColor=336791)](https://en.wikipedia.org/wiki/SQL)

### DevOps & Infrastructure
- Docker  
- AWS (EC2, S3)  
- Linux, SSH  

[![Docker](https://img.shields.io/badge/Docker-white?style=flat&logo=docker&logoColor=2496ED)](https://www.docker.com/)
[![AWS EC2](https://img.shields.io/badge/AWS_EC2-white?style=flat&logo=amazonec2&logoColor=FF9900)](https://aws.amazon.com/ec2/)
[![Amazon S3](https://img.shields.io/badge/Amazon_S3-white?style=flat&logo=amazons3&logoColor=569A31)](https://aws.amazon.com/s3/)
[![Linux](https://img.shields.io/badge/Linux-white?style=flat&logo=linux&logoColor=FCC624)](https://www.kernel.org/)
[![Ubuntu](https://img.shields.io/badge/Ubuntu-white?style=flat&logo=ubuntu&logoColor=E95420)](https://ubuntu.com/)
[![SSH](https://img.shields.io/badge/SSH-white?style=flat&logo=gnubash&logoColor=4EAA25)](https://www.openssh.com/)

---

## 🔧 Other Skills

- Webhooks & async processing  
- API performance optimization  
- Authentication & authorization (JWT, role-based access)

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Mayraju&show_icons=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Mayraju&layout=compact)

---

## 👀 Profile Views

![Visitor Count](https://visitor-badge.laobi.icu/badge?page_id=Mayraju)

---

## ⭐ Followers

[![GitHub followers](https://img.shields.io/github/followers/Mayraju?label=Follow&style=social)](https://github.com/Mayraju)








